...
//
// `POST /demo/session` is a PUBLIC, passwordless door. The finding: it used to
// hand a signed admin session to anyone who typed an email that had already
// been used for the demo, so a guessed address = full access to that visitor's
// workspace (including their uploaded RFP documents).
//
// Key invariants verified here:
//   - a claimed email with NO proof is refused (409) and mints no token
//   - a forged proof (right shape, attacker's HMAC key) is refused
//   - a VALID proof for a different visitor cannot pivot into this workspace
//   - the legitimate owner (valid or merely expired own token) still resumes
//   - a brand-new email still gets a freshly seeded workspace
//
...
/** The email already belongs to a provisioned demo visitor. */
function emailIsClaimed(): void {
...
describe('penetration: demo session takeover (SEC-1)', () => {
  it('refuses to mint a session for a claimed email with no proof', async () => {
    emailIsClaimed();

    const res = await postSession(VICTIM_EMAIL);

    expect(res.statusCode).toBe(409);
    expect(res.body).not.toContain('demo_');
    expect(res.json().token).toBeUndefined();
    // The victim's workspace must not even be touched.
    expect(mockedTransaction).not.toHaveBeenCalled();
    expect(mockedSeedOrgData).not.toHaveBeenCalled();
  });

  it('refuses a forged proof signed with an attacker key', async () => {
    emailIsClaimed();
  ...
  it('refuses a validly-signed proof issued for a different visitor', async () => {
    emailIsClaimed();
  ...
  it('refuses a proof carrying the victim org but a foreign user id', async () => {
    emailIsClaimed();
  ...
  it('does not leak the workspace owner in the refusal body', async () => {
    emailIsClaimed();
...
describe('demo session: legitimate flows still work', () => {
  it('resumes the visitor holding their own valid token', async () => {
    emailIsClaimed();
  ...
  it('provisions a fresh seeded workspace for an unclaimed email', async () => {
    ...
    mockedUserFindFirst
      ...
      .mockResolvedValueOnce({ id: 'fresh-user-id' } as never);
    const tx = { org: { create: async () => ({ id: 'fresh-org-id' }) } };
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(body.orgId).toBe('fresh-org-id');
    expect(verifyDemoToken(body.token)).toMatchObject({ u: 'fresh-user-id', o: 'fresh-org-id' });
