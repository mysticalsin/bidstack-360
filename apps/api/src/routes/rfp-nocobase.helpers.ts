/**
 * rfp-nocobase.helpers.ts — Zod schemas, shared interfaces, mock AI agents,
 * section-detection utilities, and the background intake orchestrator.
 *
 * WHY separate from rfp-nocobase.ts: these schema/type definitions and AI-agent
 * heuristics have no dependency on Fastify and can be read, tested, and
 * reasoned about independently. The route file stays focused on HTTP handlers.
 *
...
export const RfpPriority = z.enum(['low', 'medium', 'high', 'critical']);
export const RfpRecommendation = z.enum(['pending', 'bid', 'no_bid', 'conditional']);

export const RfpRequest = z.object({
  id: z.string().uuid(),
  name: z.string(),
  status: RfpStatus,
...
export const RfpDocument = z.object({
  id: z.string().uuid(),
  name: z.string(),
  fileType: z.string(),
...
export const RfpSection = z.object({
  id: z.string().uuid(),
  title: z.string(),
...
// ─── Mock Intake Agent ───────────────────────────────────────────────────────
// Replaced with real LLM call when Dust API keys are configured.

export async function mockIntakeAgent(parsedText: string): Promise<IntakeResult> {
...
// Pure keyword classifiers — no Fastify dependency, safe to unit-test in isolation.
...
export async function mockStructuringAgent(parsedText: string): Promise<SectionResult[]> {
...
// ─── Background intake orchestrator ─────────────────────────────────────────

/**
 * Runs the intake + structuring pipeline asynchronously after a document upload.
 *
 ...
 *
 * WHY exported: called fire-and-forget from the document-upload route handler.
 */
export async function runIntakeAndStructuring(
  rfpId: string,
  docId: string,
  parsedText: string,
  orgId: string,
