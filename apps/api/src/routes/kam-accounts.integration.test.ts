...
beforeAll(async () => {
  ...
  orgId = seedOrg.id;
  const user = await prisma.user.findFirst({ where: { orgId }, orderBy: { createdAt: 'asc' }, select: { id: true } });
  ...
  const other = await prisma.org.create({ data: { clerkOrg: `org_kamacct_${randomUUID().slice(0, 8)}`, name: 'KAMAcct Other' } });
  otherOrgId = other.id;
  otherCompanyId = (await prisma.company.create({ data: { orgId: other.id, name: 'oc', source: 'manual', kamStatus: 'identified' } })).id;
...
describe('KAM accounts — designate + switch', () => {
  t('a candidate appears in /candidates and NOT in /accounts', async () => {
    const cand = await server.inject({ method: 'GET', url: `/api/v1/kam/accounts/candidates?q=${NAME.slice(0, 10)}` });
  ...
  t('designate promotes off identified, stamps keyAccountSince, and flips it into /accounts', async () => {
    const res = await server.inject({
      method: 'PATCH',
      url: `/api/v1/kam/accounts/${candidateId}`,
      payload: { kamStatus: 'active', kamOwnerModel: 'presales_driven', ...(sponsorUserId ? { directorSponsorId: sponsorUserId } : {}) },
    });
  ...
  t('keyAccountSince is write-once (a later status change does not move it)', async () => {
    const before = (await server.inject({ method: 'GET', url: '/api/v1/kam/accounts' })).json()
      .items.find((a: { id: string }) => a.id === candidateId).keyAccountSince;
    const res = await server.inject({ method: 'PATCH', url: `/api/v1/kam/accounts/${candidateId}`, payload: { kamStatus: 'mapped' } });
  ...
  t('rejects designating a cross-tenant company (B3) with 404', async () => {
    const res = await server.inject({ method: 'PATCH', url: `/api/v1/kam/accounts/${otherCompanyId}`, payload: { kamStatus: 'active' } });
