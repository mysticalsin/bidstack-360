...
interface OdooPresalesKit {
  ...
  fieldMap: Array<{ odoo: string; bidstack: string; mode: 'read' | 'write' | 'enrich' }>;
  nextActions: string[];
  lastError: string | null;
}

interface OdooCompanyAutocomplete {
  generatedAt: string;
  configured: boolean;
  reachable: boolean;
  source: 'odoo' | 'local' | 'mixed' | 'none';
  items: Array<{
    id: string;
    name: string;
    ...
    phone: string | null;
    email: string | null;
    source: 'odoo' | 'company_enrichment' | 'twenty' | 'bidstack';
    confidence: number;
    matchKeys: string[];
...
export function OdooCard() {
  ...
  const kit = useQuery({
    queryKey: ['odoo:presales-kit'],
    queryFn: ({ signal }) =>
      api<OdooPresalesKit>('/api/integrations/odoo/presales-kit', { signal }),
  });
  const autocomplete = useQuery({
    queryKey: ['odoo:company-autocomplete', query],
    queryFn: ({ signal }) => {
      const params = new URLSearchParams({ q: query, limit: '5' });
      if (looksLikeDomain(query)) params.set('domain', query);
      return api<OdooCompanyAutocomplete>(`/api/integrations/odoo/company-autocomplete?${params}`, {
        signal,
      });
    },
  });
    ...
    <Card>
      ...
      {status.isLoading ? (
        ...
        <div className="px-5 py-6 text-sm text-[var(--fg-secondary)]">
          ...
          <code className="font-mono text-xs">ODOO_DB</code> to reach the mcp-server-odoo sidecar.
          Until then, BidStack uses local enrichment and verified open-data connectors.
        </div>
      ) : (
        <>
            ...
            <div className="border-b border-[var(--border-subtle)] p-3">
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-[var(--fg-secondary)]">
                    Odoo-style legal entity lookup with BidStack fallback.
                  </p>
                </div>
                <Badge
                  tone={
                    autocomplete.data?.source === 'odoo'
                      ? 'jade'
              ...
              <ul className="divide-y divide-[var(--border-subtle)]">
                {autocomplete.data?.items.map((item) => (
                  <li key={item.id} className="p-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge tone={item.source === 'odoo' ? 'jade' : 'blue'}>
                ...
                {autocomplete.data?.items.length === 0 ? (
                  <li className="p-4 text-sm text-[var(--fg-secondary)]">
                    No company suggestions yet. Add Odoo MCP credentials or enrich this account in
                    BidStack.
                  </li>
                ) : null}
              </ul>
