...
import { Stat } from '@/pages/integrations/IntegrationAtoms';
import { ConnectionCommandCenter } from '@/pages/integrations/ConnectionCommandCenter';
...
import { buildIntegrationSummary } from '@/pages/integrations/integration-helpers';
import { MCP_TOOLS } from '@/pages/integrations/types';
    ...
    <div className="space-y-5">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-6">
        ...
        <TabsContent value="agents" className="space-y-6 outline-none">
          ...
          <ApolloAccountIntelCard
            status={connectors.data?.items.find((item) => item.id === 'apollo-organizations')}
...
function ApolloAccountIntelCard({
  status,
  isLoading,
}: {
  status:
  ...
  const envSnippet = [
    'APOLLO_MCP_URL=https://...',
    'APOLLO_MCP_BEARER_TOKEN=...',
    'APOLLO_API_KEY=...',
    'APOLLO_API_ENABLE_PEOPLE_SEARCH=true',
      ...
      <SectionHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <a
              href={
                status?.docsUrl ?? 'https://docs.apollo.io/docs/apollo-mcp-server-documentation'
              }
              target="_blank"
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {[
              ...
              <div
                key={label}
                className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-secondary)] px-3 py-2"
              >
                <div className="text-[10px] font-semibold uppercase tracking-wider text-[var(--fg-tertiary)]">
                  {label}
                </div>
                <div className="mt-1 text-sm font-medium text-[var(--fg-primary)]">{value}</div>
              </div>
            ))}
          </div>
          {status?.message ? (
            <p className="rounded-lg bg-[var(--surface-secondary)] px-3 py-2 text-xs text-[var(--fg-secondary)]">
              {status.message}
            </p>
          ) : null}
        </div>
