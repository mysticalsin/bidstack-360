...
export function CommandPalette({ open, onOpenChange }: CommandPaletteProps) {
  ...
  // Server-side opportunity search — only fires when there's a query.
  const oppSearch = useQuery({
    enabled: open && query.trim().length >= 2,
    queryKey: ['palette:opps', query],
    queryFn: ({ signal }) =>
      api<OpportunityPage>(
        `/api/opportunities?search=${encodeURIComponent(query)}&limit=8`,
        { signal },
      ),
  });
  ...
  const items: Item[] = useMemo(() => {
    ...
    // Navigation always shows; filtered locally by label match.
    ...
    // Opportunity matches from the API.
    ...
    return out;
  }, [query, oppSearch.data, navigate, onOpenChange]);

  // Clamp active index when items shrink.
  useEffect(() => {
