...
| CI repeat evidence gates | `evidence/2026-07-01/ci-repeat-release-evidence-gates.log` | Gate 8/9 - strict release evidence now requires 10 consecutive full-suite CI runs on isolated pgvector Postgres tied to the release commit/branch; local gate selftests plus root `pnpm test` passed |
| Test hermeticity guard | `pnpm test:hermeticity` / `scripts/verify-test-hermeticity.mjs` | Gate 9 - root `pnpm test` now fails before the suite if API tests reintroduce shared `org_seed_mantu` tenant references |
| Release preflight action list | `deploy-evidence/release-preflight-latest.json` / `pnpm deploy:evidence:preflight:production` | Gate 4/7/9/11/13 - preflight now writes `preflightSummary` and `preflightActionItems` with exact acceptable env names grouped by operator area; the current shell still correctly blocks on missing live inputs |
...
- API/MCP release identity hardening (uncommitted): live API/MCP connectivity artifacts now require health/discovery release identity to match source-control evidence so stale deployments cannot pass the release gate.
- Release preflight operator checklist (uncommitted): `deploy:evidence:preflight:*` now writes `preflightSummary` and `preflightActionItems` so missing live API/MCP, CI, ops, PII, webhook, Sentry, browser, load, secret, and container inputs are grouped with exact acceptable env var names.
