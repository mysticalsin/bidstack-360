...
> The `BIDSTACK_*` `deploy:evidence` env vars (release id, reviewer, approver, ops-readiness file, container scan images, PII/webhook ciphertext DB URLs, load/API/sentry/a11y/browser/MCP targets and tokens) are inputs to the gate bundle (§7), not app runtime config.
...
| api        | `deploy:evidence:api`               | Live API connectivity — non-local API target + authenticated token; proves `/livez`, `/readyz` (DB + Redis + storage), `/health` (DB + Redis), `/api/me/capabilities` with user/org tenant context, and a no-match `GET /api/companies` domain read smoke without storing raw rows                                               |
