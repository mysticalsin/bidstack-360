...
- **PII at rest:** `PII_FIELD_ENCRYPTION` (default OFF — set `true`), `PII_ENCRYPTION_MASTER_KEY` (see `PRODUCTION_READINESS_REPORT.md` Gate 5/13; run `scripts/encrypt-existing-pii.ts`, then `pnpm deploy:evidence:pii` against the release DB). **REQUIRES ACCESS** (prod secret store + DB access + decision).
...
> The `BIDSTACK_*` `deploy:evidence` env vars (release id, reviewer, approver, ops-readiness file, container scan images, PII/webhook ciphertext DB URLs, load/API/sentry/browser/MCP targets and tokens) are inputs to the gate bundle (§7), not app runtime config.
...
| api        | `deploy:evidence:api`               | Live API connectivity — non-local API target + authenticated token; proves `/livez`, `/readyz` (DB + Redis + storage), `/health` (DB + Redis), and `/api/me/capabilities` with user/org tenant context                                                                                                                |
| pii        | `deploy:evidence:pii`               | Raw DB PII ciphertext proof — privacy-safe counts only; proves `Contact`, `Lead`, and `KamConsultant` PII columns are `enc:v1:*`, email hashes are present/valid, and no plaintext rows remain                                                                                                                        |
...
| mcp        | `deploy:evidence:mcp`               | Live MCP connectivity — non-local Streamable HTTP target + bearer token; proves `/.well-known/mcp`, `/health`, `initialize`, `notifications/initialized`, `tools/list` includes required tools, and one privacy-safe read-only `tools/call` succeeds                                                                  |
...
| sentry     | `deploy:evidence:sentry:trigger`    | Sentry smoke — triggers a real error to a non-local target + verifies the issue surfaces                                                                                                                                                                                                                              |
...
API evidence inputs: `BIDSTACK_API_CONNECTIVITY_TARGET` (or `BIDSTACK_API_BASE_URL` / `API_BASE_URL`), `BIDSTACK_API_CONNECTIVITY_API_TOKEN` (or `BIDSTACK_API_KEY` / `API_TOKEN`), optional `BIDSTACK_API_CONNECTIVITY_AUTH_SCHEME=api-key|bearer`, and optional `BIDSTACK_API_CONNECTIVITY_EXPECTED_ORG_ID`.

PII ciphertext evidence inputs: `BIDSTACK_PII_CIPHERTEXT_DATABASE_URL` (or `DATABASE_URL`), `PII_FIELD_ENCRYPTION=true`, `PII_ENCRYPTION_MASTER_KEY`, optional `BIDSTACK_PII_CIPHERTEXT_REQUIRED_MODELS=contact,lead,kamConsultant`, and optional `BIDSTACK_PII_CIPHERTEXT_EVIDENCE`.
...
Required MCP tools default to `opportunities.list`, `contacts.list`, `tasks.list`, `crm_search_companies` (override via `BIDSTACK_DEPLOY_REQUIRED_MCP_TOOLS` or `BIDSTACK_MCP_CONNECTIVITY_REQUIRED_TOOLS`). The default tool-call smoke is `crm_search_companies` with a no-match query and `limit:1`; override only with another read-only tool via `BIDSTACK_MCP_CONNECTIVITY_SMOKE_TOOL` and JSON-object `BIDSTACK_MCP_CONNECTIVITY_SMOKE_ARGS`. MCP evidence inputs: `BIDSTACK_MCP_CONNECTIVITY_TARGET` (or `BIDSTACK_MCP_PUBLIC_URL` / `DUST_MCP_PUBLIC_URL` / `MCP_BASE_URL`) and `BIDSTACK_MCP_CONNECTIVITY_API_TOKEN` (or `MCP_API_TOKEN` / `API_TOKEN`). The artifact stores tool names, status, result-shape counts, and privacy flags only; it must not store raw arguments or tool output.
...
7. Sentry smoke + cross-role browser regression against a live target — §7.
8. Container + SAST + secret scans run in CI with a Docker daemon — §7, Gates 7/8.
9. `PII_FIELD_ENCRYPTION=true` + master key decision + `pnpm deploy:evidence:pii` release DB proof — §2.7/§7, Gate 5/13.
