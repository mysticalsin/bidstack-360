...
2. **webhook plaintext-secret-in-production** — fixed 2026-06-28 in current WIP by moving webhook runtime to `decryptWebhookSigningSecret`, which reads the fallback env live, allows only legacy `whsec_` plaintext outside production-like runtimes (or with an explicit temporary flag), and rejects malformed values. Covered by `packages/shared/src/utils/crypto.test.ts`, `apps/worker/src/queues/webhook-delivery.test.ts`, and the new webhook backfill/evidence selftests.
